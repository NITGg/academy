...
The search script lives inside this skill's own directory. Paths below are relative to the project root, so run them from there:

```bash
py -3 ".claude/skills/ui-ux-pro-max/scripts/search.py" "<query>" --domain <domain>
```

On this machine only the `py -3` launcher works (`python` / `python3` hit the Windows Store alias stub). Requires Python 3.x, no external dependencies.
...
# Step 3: supplement
py -3 ".claude/skills/ui-ux-pro-max/scripts/search.py" "search loading animation" --domain ux
