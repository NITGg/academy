...
→ `in_progress`, records `actual_start`.

### Complete — `complete_lesson` (POST) · teacher
Params: `lessonid`, `note?`. From `confirmed`/`in_progress`. Consumes the reserved Flex.
→ `completed`, records `actual_end`. *(Revenue split US-FN-1-4 is Phase 3.)*
...
Consumes the Flex. → `student_absent`.

### Teacher absent — `report_teacher_absent` (POST) · student
Params: `lessonid`. From `confirmed`/`in_progress`, after `absence_report_minutes` past start.
Returns the Flex. → `teacher_absent`.
...
`start_allowed_minutes` · `absence_report_minutes`. Read them with `get_lesson_settings`.
