...
const site = folder('02 · Site Content (public, no login)',
  ...
  ws('Home hero text (headline lines + paragraph)', 'local_nit_category_get_home_hero', { token: '{{regtoken}}', nologin: true, params: [O('lang', '{{lang}}', 'en / ar')], desc: 'The front-page hero as data: `headline[]` one entry per line (paint the last line in the accent colour), `title` joined, `description` the paragraph. Already resolved to one language.\n\nDoc: docs/apis/home-hero-mobile-api.md' }),
  ...
  ws('Static pages — list', 'local_profilefields_get_static_pages', { token: '{{regtoken}}', nologin: true, params: [O('lang', '{{lang}}', 'en / ar')], desc: 'Slug, kind, name and web address of every published static page (about, contact, terms, privacy, refund, faq).\n\nDoc: docs/apis/static-pages-mobile-api.md' }),
  ws('Static page — one page', 'local_profilefields_get_static_page', { token: '{{regtoken}}', nologin: true, params: [R('page', 'about', 'about | contact | terms | privacy | refund | faq'), O('lang', '{{lang}}', 'en / ar')], desc: 'Body as HTML, plus contact rows/social links (Contact) or Q&A (FAQ). Legal pages return the mapped tool_policy document. `page=about` adds an `about` block: tagline, lede, heroimage, video {provider, url, mimetype} (an uploaded file - play it natively, picture as poster), facts, pillars, milestones, coursesurl, contacturl.\n\nDoc: docs/apis/static-pages-mobile-api.md' }),
...
const catalogue = folder('03 · Catalogue & Courses',
  ...
  ws('Course details (product / preview screen)', 'core_course_get_courses_by_field', { params: [R('field', 'id', 'id | ids | shortname | idnumber | category'), R('value', '{{courseid}}', '')], desc: 'Core. Works without enrolment. `overviewfiles[0].fileurl` (+`&token=`) is the cover, `contacts[]` the instructors, `customfields[]` the prerequisites / ILOs / hours / language — read `valueraw` for number. The Free and Certificate flags are NOT custom fields any more: read them from `is_course_free`.' }),
  ...
  api('Is course free?', 'is_course_free', { params: [R('courseid', '{{courseid}}', '')], desc: '`{courseid,is_free,has_certificate,price?,currency?,country_required?}`. Both flags are computed by the server: `is_free` = no active price rule, `has_certificate` = the course contains a visible certificate activity. They replaced the old `free` / `certificate` checkbox custom fields (removed 2026-09-13).' }),
...
const learning = folder('05 · Learning (inside a course)', 'Playing lessons, taking quizzes, asking the AI assistant and filling Job Forms.', [
  folder('Secure video (VdoCipher)', 'DRM video: the server mints a short-lived, watermarked OTP per play. Fetch it **at tap time**, never cache it.\n\nDoc: docs/apis/vdocipher-mobile-api.md', [
    api('Get playback OTP', 'get_playback', { plugin: 'vdocipher', params: [R('cmid', '{{cmid}}', 'Course module id of the VdoCipher activity (or just GET the activity\'s `otpurl`).')], desc: 'Success → `{videoid, otp, playbackInfo, watermark, ttl, viewed, completionstate}` — `otp` + `playbackInfo` go to `EmbedInfo.streaming`. Minting also **marks the activity viewed** for the user (what `mod/vdocipher/view.php` does on the website), so an "automatic · require view" completion turns complete and anything restricted on it unlocks: `viewed` true, `completionstate` 1 = complete (null when completion is off for that activity). Fail → not enrolled / no access / no video attached.' }),
    api('Mark video viewed (no OTP)', 'mark_viewed', { plugin: 'vdocipher', params: [R('cmid', '{{cmid}}', 'Course module id of the VdoCipher activity.')], desc: 'Records the view without minting an OTP — `get_playback` already does this, so most apps never call it; it exists for re-recording (e.g. when the player closes). Idempotent. Success → `{cmid, viewed, completionstate}`.' }),
  ]),
