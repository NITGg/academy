...
### 3.0 تدفّق شراء باقة B2B جديدة (من قسم "الاشتراكات" — زر "اشتراك B2B")

عند تصفّح قسم "الاشتراكات" في الموقع (`http://localhost:3000/subscriptions` أو قسم "الاشتراكات" في الصفحة الرئيسية):

```
1) get_available_subscriptions               ← جلب كافة الباقات المتاحة
     └─ فحص b2b_enabled === 1                ← يظهر زر "اشتراك B2B" في كارت الباقة
2) عند النقر على "اشتراك B2B"                 ← فتح BuySubscriptionModal (نافذة اختيار المقاعد)
     ├─ اختيار عدد المقاعد (seats)
     └─ preview_discount (اختياري)            ← معاينة السعر الفردي والإجمالي والخصم/الكوبون
3) عند النقر على تأكيد الشراء / الدفع
     └─ create_subscription_checkout (POST)  ← إرسال subscriptionid و type="b2b" و seats
          └─ checkout_url                    ← إعادة توجيه المستخدم تلقائيًا لبوابة الدفع (Kashier)
4) بعد إتمام الدفع والعودة
     └─ get_my_b2b_subscriptions             ← التعرّف على الاشتراك الجديد المملوك
          └─ تحوّل الزر إلى "إدارة اشتراك B2B" ظهور تبويب "إدارة اشتراك B2B"
```

### 3.1 تدفّق إدارة اشتراكات B2B بعد الشراء
...
على الموبايل: نفس التسلسل تمامًا — يمكنك استخدام `get_available_subscriptions` و `create_subscription_checkout` لتنفيذ تدفّق الشراء، ثم `get_my_b2b_subscriptions` و `get_b2b_dashboard` لإدارة الاشتراك.

### 3.2 وضع «الاطّلاع فقط» (read-only history) — إلزامي

الإدارة (توليد/إلغاء رابط، قبول/رفض/إزالة عضو) مسموحة **فقط طالما الاشتراك نشِط** (`status === 'active'`).
بمجرد انتهاء الاشتراك أو إلغائه يصبح **سجلًّا للاطّلاع فقط**: يظهر في القائمة ويمكن فتحه لعرض الأعضاء
والإحصائيات والتاريخ، لكن **لا يجوز أي إجراء إداري عليه**.

استخدم الحقل `can_manage` (الموجود في `get_my_b2b_subscriptions` وفي `get_b2b_dashboard().capacity`)
لتحديد الوضع. عندما `can_manage === false`:

- أخفِ **قسم رابط الدعوة بالكامل** (الرابط نفسه، وزر **«نسخ»**، و«توليد»، و«إلغاء») — الاشتراك المنتهي
  ليس له رابط قابل للمشاركة، فلا نسخ ولا توليد ولا إلغاء.
- أخفِ أزرار **«قبول» / «رفض» / «إزالة»** في جدول الأعضاء.
- اعرض شريطًا يوضّح أنّ الاشتراك انتهى ويُعرض كسجلّ.

> **الخادم يفرض هذا أيضًا:** حتى لو استُدعي أي إجراء إداري على اشتراك غير نشِط سيردّ الخادم بخطأ
> (`err_b2bnotactive` أو `err_b2bexpired`). فإخفاء الأزرار في العميل هو لتحسين تجربة المستخدم، والخادم
> هو مصدر الحقيقة. لا تعتمد على إخفاء الأزرار وحده — تعامَل دائمًا مع هذين الخطأين.

---

## 4. مرجع الـ APIs

### 4.0 APIs شراء وتجهيز باقة B2B

#### 4.0.1 `get_available_subscriptions` — جلب الباقات المتاحة وتفاصيل B2B
- **Method:** GET · **Params:** لا شيء (غير التوكن).
- **Response `data`:** مصفوفة من الباقات المتاحة، كل عنصر يحتوي على تفاصيل الباقة وما إذا كانت تدعم B2B وخيارات المقاعد:
```json
[
  {
    "id": 7,
    "name": "الباقة الاحترافية",
    "description": "اشتراك شامل لجميع كورسات الأكاديمية",
    "price": "400.00",
    "duration_days": 30,
    "b2b_enabled": 1,              // 1 = تتيح B2B (يظهر زر "اشتراك B2B")، 0 = فردي فقط
    "seat_options": [              // خيارات الفئات وخصومات المقاعد (إن وجدت)
      { "seats": 10, "discount_percent": 0 },
      { "seats": 25, "discount_percent": 10 }
    ]
  }
]
```
- **الاستخدام:** في قسم "الاشتراكات" (`http://localhost:3000/subscriptions`) لعرض الكروت وإظهار زر **"اشتراك B2B"** للباقات التي تحتوي على `b2b_enabled: 1`.

#### 4.0.2 `preview_discount` — معاينة خصم الباقة والكوبون
- **Method:** GET · **Params:** `item_type=subscription`, `item_id=<subscription_id>`, `coupon_code?=<كود_الكوبون>`.
- **Response `data`:**
```json
{
  "original": 400,
...
```
- **الاستخدام:** يُستدعى أثناء فتح نافذة الشراء (`BuySubscriptionModal`) لحساب السعر الفردي الصافي بعد التخفيضات والكوبون، وتطبيقه على إجمالي المقاعد المطلوبة (`final × seats`).

#### 4.0.3 `create_subscription_checkout` — إنشاء عملية شراء/دفع باقة B2B
- **Method:** POST · **Params (Form Data / Body):**
  - `subscriptionid` (int، مطلوب): مُعرّف الباقة المراد شراؤها.
  - `type` (string، مطلوب): `"b2b"` (**حاسم جداً لتحديد نوع الشراء كاشتراك شركات وليس اشتراكاً فردياً `"normal"`**).
  - `seats` (int، مطلوب في حالة B2B): عدد المقاعد المطلوبة (مثال: `10` أو `25`).
  - `coupon_code` (string، اختياري): كود الخصم كالتأكيد.
  - `return_url` (string، اختياري): رابط العودة بعد النجاح/الإلغاء في بوابة الدفع.
- **Response `data`:**
```json
{
  "checkout_url": "https://kashier.io/checkout/..."
}
```
- **الاستخدام:** عند الضغط على تأكيد الشراء في نافذة `BuySubscriptionModal` زر "اشتراك B2B". يتم استلام `checkout_url` وتوجيه متصفح المستخدم تلقائيًا إليه لإتمام عملية الدفع.
...
[
  {
    ...
    "can_manage": true,        // ← true فقط عندما status = active (اشتراك نشِط)
...
- **`can_manage`:** إذا كانت `false` (اشتراك منتهٍ/ملغى) فالاشتراك **للاطّلاع فقط (سجلّ)** — راجع القسم 3.1.
...
```json
{
  "capacity": {
    ...
    "status": "active",
    "can_manage": true         // ← إن كانت false: أخفِ كل أزرار الإدارة واعرض الشاشة كسجلّ (القسم 3.1)
  },
  "members": [
    {
...
| `err_b2bnotactive` / `err_b2bexpired` | الاشتراك غير نشِط/منتهٍ | امنع **كل** الإجراءات الإدارية (توليد/إلغاء رابط، قبول/رفض/إزالة) واعرض الشاشة كسجلّ — القسم 3.1 |
...
|---|---|---|---|
| `get_available_subscriptions` | GET | — | جلب قائمة الباقات المتاحة وتفاصيل `b2b_enabled` و `seat_options` |
| `preview_discount` | GET | `item_type=subscription`, `item_id`, `coupon_code?` | معاينة الأسعار والخصومات والكوبونات قبل الشراء |
| `create_subscription_checkout` | POST | `subscriptionid`, `type="b2b"`, `seats`, `coupon_code?`, `return_url?` | بدء عملية الدفع لشراء باقة B2B وإنشاء رابط Kashier |
...
كل الـ POST ترجع `{ status:'success', data:{ ... } }`. أضِف `token` لكل طلب و`alang=ar|en` للترجمة.

---

**مراجع الكود:**
- قسم الاشتراكات في Frontend: [SubscriptionCatalog.tsx](file:///d:/My%20work/NIT/Projects/Academy/student-frontend/src/features/subscriptions/components/SubscriptionCatalog.tsx)
- نافذة تحديد المقاعد والشراء: [BuySubscriptionModal.tsx](file:///d:/My%20work/NIT/Projects/Academy/student-frontend/src/features/subscriptions/components/BuySubscriptionModal.tsx)
- سيرفر أفعال الشراء (Actions): [actions.ts](file:///d:/My%20work/NIT/Projects/Academy/student-frontend/src/features/subscriptions/actions.ts)
- الصفحة الخلفية للداشبورد: [b2b_dashboard.php](../src/local/academy/b2b_dashboard.php)
