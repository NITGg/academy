...
$string['guest_userid'] = 'Guest browsing account (user id)';
$string['guest_userid_desc'] = 'The id of the account the guest browsing token belongs to. Prices treat that account\'s calls as a visitor\'s (placed by IP address, then by the country the app sends, then the Default price) instead of as a member with no profile country — otherwise every pre-login call gets price 0 and "set your country". Leave 0 when the account was created by <code>cli/app_guest_token.php</code>, which marks it with the <code>nit_app_guest</code> role instead.';
