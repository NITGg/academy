/*
 * NIT Flex shared front-end helpers: a client-side paginator and a search-and-pick combobox.
 * Loaded as a plain (non-AMD) script via $PAGE->requires->js('/local/nit_flex/ui.js', true) so the
 * per-page inline IIFEs can reuse it. Ported from the reference local_academy ui.js.
 */
(function (w) {
    'use strict';

    var DEFAULT_LABELS = { info: 'Showing {from}–{to} of {total}', prev: '‹', next: '›' };
    ...
    w.NitUI = w.NitUI || {};
    w.NitUI.paginate = paginate;
    w.NitUI.picker = picker;
    w.NitUI.userPicker = userPicker;
})(window);
