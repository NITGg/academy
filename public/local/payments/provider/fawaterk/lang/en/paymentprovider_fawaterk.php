...
$string['auth_heading_desc'] = 'How Moodle authenticates its calls to the Fawaterk API. Both credential sets come from the Fawaterk dashboard: <b>Integrations</b> in the left menu. Note that OAuth clients can only be created on the live dashboard, so OAuth and sandbox mode cannot be combined.';
$string['auth_mode'] = 'Authentication method';
$string['auth_mode_desc'] = 'Use the HASH API key: it is what the v2 payment endpoints accept. The OAuth option issues tokens correctly, but those tokens are rejected by the payment API (verified against a live account) &mdash; they belong to the newer Fawaterk Integrations API. Only switch to OAuth if Fawaterk confirms the payment endpoints now accept it.';
$string['auth_mode_oauth'] = 'OAuth 2.0 client credentials (not accepted by the payment API yet)';
$string['auth_mode_apikey'] = 'HASH API key (works &mdash; use this)';
