...
$string['currencies'] = 'Settlement currencies';
$string['currencies_desc'] = 'Comma-separated currency codes Fawaterk may be offered for. The buyer is charged in the order\'s own currency, so a USD order shows as "Pay USD 4.50" on the payment page; only Fawaterk\'s reporting converts to the account\'s base currency, which is handled. Narrow this list if the account genuinely cannot take one of these — a course priced in a currency that is not listed falls through to another provider instead.';
$string['log_api_calls'] = 'Log every API call';
$string['log_api_calls_desc'] = 'Record the exact request and the exact response body for every Fawaterk call, viewable with <code>fawaterk_diagnose.php --logs</code>. Failed calls are always logged this way; this setting adds the successful ones too. The Authorization header is never recorded, but the request body carries the buyer\'s name, email, phone and address — turn this off once you have finished debugging.';
$string['accept_converted'] = 'Accept payments Fawaterk converted';
$string['accept_converted_desc'] = 'Allow a payment to complete even when Fawaterk settled it in a different currency from the order. Off by default, because it means trusting the Fawaterk exchange rate for money the site did not price: a 4.50 USD order settles as roughly 240 EGP and the two can no longer be reconciled. With this on, the order keeps the price the buyer agreed to and the settled figure is recorded beside it. Prefer pricing in a currency the account settles in.';
