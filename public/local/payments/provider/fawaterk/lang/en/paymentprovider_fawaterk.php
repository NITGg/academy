...
$string['currencies_desc'] = 'Comma-separated currency codes this Fawaterk account settles in natively. Fawaterk will accept an order in another currency and silently convert it, so a 4.50 USD order can arrive as 240.435 EGP — Moodle then compares its own 4.50 against their converted total and rejects a payment the buyer really made. Fawaterk is only offered for the currencies listed here; a course priced in anything else falls through to another provider. Widen this only once the account genuinely settles in that currency.';
