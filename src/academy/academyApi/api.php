...
function create_bbb_table($message,$messageKey,$hasBeenForciblyEnded,$duration,$hasUserJoined,$createDate,$dialNumber,$voiceBridge,$createTime,$moderatorPW,$attendeePW,$parentMeetingID,$internalMeetingID,$meetingID,$courseid,$isRunning){
		...
		function create_new_bbb_session($course,$user){
			...
			if($available=="true"){
			   ...
			   $logoutURL=$CFG->wwwroot."/bbb/end.php?course=".$course;
				 ...
				 $value=sha1($link);
				 $response=ACADEMY_BBB_URL.'create?name='.$name.'&meetingID='.$meetingID.'&attendeePW='.$attendeePW.'&moderatorPW='.$moderatorPW.'&maxParticipants='.$maxUsers.'&welcome='.$welcome.'&copyright='.$copyright.'&duration='.$duration.'&logoutURL='.$logoutURL.'&checksum='.$value.'';
