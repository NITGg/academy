...
if (!empty($token)) {
    ...
    try {
        ...
        if (!empty($array)) {
            ...
            if ($function == 'add_course_rate') {
                echo add_course_rate($userID, $courseId, $rate);
            }
...
function teachers()
{
    ...
    $array = array();
    $teachers = $DB->get_records_sql("SELECT DISTINCT u.id ,u.firstname,u.lastname  FROM mdl_user as u INNER JOIN mdl_role_assignments as role ON role.userid=u.id and role.roleid=3");
    $fs = get_file_storage();
    foreach ($teachers as $teacher) {
        ...
        $url = upload_user_image($teacher->id);
...
function forget_password($username)
{
    global $DB, $OUTPUT, $CFG;
    $user = "";
    $userData = $DB->get_records_sql("SELECT * FROM {user} WHERE username = :uname OR email = :email", array('uname' => $username, 'email' => $username));
    ...
    if (!empty($user)) {
        ...
        $message = 'You should go to these link to change password <a href= ' . $CFG->wwwroot . '/academy/academyApi/confirm.php>Confirm Password</a>';
        $phpmailer->Body = $message;
        $phpmailer->IsHTML(true);
        if (!$phpmailer->send()) {
            return json_encode(["status"=>"fail","message" =>"",'error'=> "Error: " . $phpmailer->ErrorInfo]);

            // echo "Mailer Error: " . $phpmailer->ErrorInfo;
        } else {
            echo json_encode(["status"=>"success","message" => "Sent",'error'=>""], true);
        }
    } else {
        echo json_encode(["status"=>"fail","message" => "",'error'=>"username is not exist"]);
    }
}
...
{
    global $DB;
    $feedbacks_raw = $DB->get_records_sql(
        "SELECT f.*, u.id as tid, u.firstname, u.lastname, u.email, u.url as picture
         FROM {feedbacks} f
         LEFT JOIN {user} u ON f.teacher_id = u.id
         WHERE f.user = :userid",
        array('userid' => $userID)
    );
    if (empty($feedbacks_raw)) {
        return json_encode(["feedbacks" => "no feedbacks"]);
    }
    $feedbacks = array();
    foreach ($feedbacks_raw as $fb) {
        $fb->teacher = (object)array(
            'id' => $fb->tid,
        ...
        unset($fb->tid, $fb->firstname, $fb->lastname, $fb->email, $fb->picture);
        $feedbacks[] = $fb;
    }
...
function get_teacher_profile_data($teacherID, $userid)
{
    ...
    $getTeacher = $DB->get_records_sql('SELECT * FROM {user} WHERE id = :id', array('id' => $teacherID));
    ...
    $feedBacks = array();
    $checkfeedback = $DB->get_records_sql("SELECT * FROM {feedbacks} WHERE teacher_id = :tid", array('tid' => $teacherID));
    ...
    if (empty($getTeacher[0]->rating)) {
        $getTeacher[0]->rating = "0";
    }
...
function get_user_by_id($userId)
{
    ...
    if (!empty($user)) {
        return json_encode(["user" => $user->firstname." ".$user->lastname]);
    }
    return json_encode(["error" => 'no user found']);
}
...
{
    global $DB, $OUTPUT, $CFG;
    $user = $DB->get_records_sql("SELECT * FROM {message_contact_requests} WHERE userid = :uid AND requesteduserid = :rid", array('uid' => $id, 'rid' => $requestedid));
...
function get_enrol_courses($id)
{
    ...
    foreach ($EnroledCourses as $course) {
        ...
        $overviewfiles = array();
			...
			$course->overviewfiles = $overviewfiles[0]['fileurl'];
        }
    ...
    return json_encode(["courses" => array_values($EnroledCourses)]);
...
{
    global $DB;
    $DB->delete_records('vimeovedios', array('id' => $id));
...
{
    global $DB;
    $DB->delete_records('teachersphotos', array('id' => $id));
...
function upload_image($id)
{
    ...
    $postImage = rand(0, 1000) . "_" . $postImageName;
    $uploadFiles = move_uploaded_file($postImageTemp, $CFG->dirroot . "/theme/edumy/images/teachers/" . $postImage);
    $checkUser = $DB->get_record('user', array('id' => $id));
    if ($uploadFiles) {
        $DB->execute("UPDATE {user} SET url = :img WHERE id = :id", array('img' => $postImage, 'id' => $id));
...
{
    global $DB, $OUTPUT, $CFG;
    $getTeacher = $DB->get_records_sql('SELECT firstname,lastname FROM mdl_user WHERE id=' . $teacherid . ' ');
    $getTeacher = array_values($getTeacher);

    $url = upload_user_image($teacherid);
...
function edit_user_data($userid, $firstname, $lastname, $phone1, $phone2)
{
    ...
    try {
        ...
        if (!empty($phone1)) { $user->phone1 = $phone1; }
        if (!empty($phone2)) { $user->phone2 = $phone2; }
...
function get_user_name($userId)
{
    global $DB, $OUTPUT, $CFG;
    //$userData = get_complete_user_data('id', $teacherID);
    $getUser = $DB->get_records_sql('SELECT firstname,lastname,username,phone1,phone2 FROM {user} WHERE id = :id', array('id' => $userId));
...
{
    global $DB, $OUTPUT, $CFG;
    $years = $DB->get_records_sql("SELECT * FROM {teacher_years} WHERE teacherID = :tid", array('tid' => $teacherId));
...
function get_h5p_result($courseId)
{
    global $DB, $OUTPUT, $CFG;
    $h5p = new stdClass();
    $h5p = $DB->get_records_sql("SELECT * FROM {hvp} WHERE course = :cid", array('cid' => $courseId));
...
{
    global $DB;
    $courses = $DB->get_records_sql("SELECT * FROM {course} WHERE category = :catid", array('catid' => $categoryid));
...
function create_child($email, $parentid, $phone1, $check = 0)
{
    ...
    if ($isStudent) {
        ...
        return json_encode(['data' => 'Error the user is not a student']);
    }
}
...
function get_parent_data($parent)
{
    ...
    foreach ($childs as $child) {
        $child->url = upload_user_image($child->childid);
    }
...
function _get_courses_data($user, $categoryfilter = null)
{
    global $DB, $CFG;

    $sql = "SELECT co.id as courseId, co.visible as visible, co.fullname as courseName,
            co.summary as courseDesc, co.category as catId, cat.name as catName
            FROM {course} co
            JOIN {course_categories} cat ON co.category = cat.id
            WHERE co.visible = 1";
    $params = array();
    if ($categoryfilter !== null) {
        $sql .= " AND co.category = :catid";
        $params['catid'] = $categoryfilter;
    }
    ...
    $prices_raw = $DB->get_records_sql(
        "SELECT instanceid, value FROM {customfield_data} WHERE fieldid = :fid AND instanceid $insql",
        array_merge(['fid' => 12], $inparams)
    );
    $prices = array();
    foreach ($prices_raw as $p) { $prices[$p->instanceid] = $p->value; }

    $descs_raw = $DB->get_records_sql(
        "SELECT instanceid, value FROM {customfield_data} WHERE fieldid = :fid AND instanceid $insql",
        array_merge(['fid' => 15], $inparams)
    );
    $descs = array();
    foreach ($descs_raw as $d) { $descs[$d->instanceid] = $d->value; }

    list($insql2, $inparams2) = $DB->get_in_or_equal($courseids, SQL_PARAMS_NAMED);
    $ratings_raw = $DB->get_records_sql(
        "SELECT courseid, CEIL(AVG(rate)) as rate FROM {course_rates} WHERE courseid $insql2 GROUP BY courseid",
        $inparams2
    );
    $ratings = array();
    foreach ($ratings_raw as $r) { $ratings[$r->courseid] = $r->rate; }

    list($insql3, $inparams3) = $DB->get_in_or_equal($courseids, SQL_PARAMS_NAMED);
    $views_raw = $DB->get_records_sql(
        "SELECT courseid, visit FROM {course_views} WHERE courseid $insql3",
        $inparams3
    );
    $views = array();
    foreach ($views_raw as $v) { $views[$v->courseid] = $v->visit; }

    list($insql4, $inparams4) = $DB->get_in_or_equal($courseids, SQL_PARAMS_NAMED);
    $teachers_raw = $DB->get_records_sql(
        "SELECT c.id as courseid, u.id as teacherid, u.firstname, u.lastname
         FROM {course} c
         JOIN {context} cx ON c.id = cx.instanceid AND cx.contextlevel = 50
         JOIN {role_assignments} ra ON cx.id = ra.contextid AND ra.roleid = 3
    ...
    foreach ($teachers_raw as $t) {
        $teachers[$t->courseid] = array('id' => $t->teacherid, 'name' => $t->firstname . ' ' . $t->lastname);
    }
    ...
    foreach ($courses as $course) {
        ...
        $enrol = is_enrolled($context, $user, '', true) ? 'true' : 'false';

        $overviewfiles = '';
        foreach ($courselist->get_course_overviewfiles() as $file) {
            $overviewfiles = moodle_url::make_webservice_pluginfile_url(
        ...
        $tid = isset($teachers[$course->courseid]) ? $teachers[$course->courseid]['id'] : -1;
        $tname = isset($teachers[$course->courseid]) ? $teachers[$course->courseid]['name'] : null;

        $coursesData[] = array(
            'course_id' => $course->courseid,
            'course_name' => $course->coursename,
            'enrol' => $enrol,
            'course_desc' => isset($descs[$course->courseid]) ? $descs[$course->courseid] : '',
            'views' => isset($views[$course->courseid]) ? $views[$course->courseid] : 0,
            'teacherId' => $tid,
            'teacherName' => $tname,
            'image' => $overviewfiles,
            'price' => isset($prices[$course->courseid]) ? $prices[$course->courseid] : 'free',
            'rate' => isset($ratings[$course->courseid]) ? $ratings[$course->courseid] : 0,
            'cat_id' => $course->catid,
            'cat_name' => $course->catname
        );
    }
            ...
            if (empty($filters['excludemodules']) and !empty($modinfosections[$section->section])) {
                foreach ($modinfosections[$section->section] as $cmid) {
                    ...
                    if ($module['avail'] == false) {
                        ...
                        $string = '';
                        // $module['avail_message'] =\core_availability\info::format_info($cm->availableinfo, $course);
                        if (gettype($reason) == 'string') {

                            $module['avail_message'] = strip_tags(\core_availability\info::format_info($cm->availableinfo, $course));
                        } else {
                            $string = get_string('list_root_and', 'theme_edumy');
                            for ($i = 0; $i < count($reason->items); $i++) {
                                // array_push($module['avail_message'], $reason->items[$i]->text." <br>");
                                $string .= strip_tags(\core_availability\info::format_info($reason->items[$i], $course)) . ",";
                            }
                    ...
                    if ($cm->modname == "resource") {
                        ...
                        } elseif ($video_type->type == "1") {
                            $module['resource_type'] = "quiz";
                        } elseif ($video_type->type == "3") {
                            $module['resource_type'] = "homework";
                        } elseif ($video_type->type == "4") {
                            $module['resource_type'] = "summary";
                        } elseif ($video_type->type == "5") {
                            $module['resource_type'] = "revision";
                        }
                    }
                    $module['page_url'] = "";
...
function course_content_mobile($course, $userid)
{
    ...
    $check_type = $DB->get_record('customfield_data', array('fieldid' => 28, 'instanceid' => $course));
    $requirements = $DB->get_record('customfield_data', array('fieldid' => 13, 'instanceid' => $course));
    $forwhom = $DB->get_record('customfield_data', array('fieldid' => 14, 'instanceid' => $course));
    $benefits = $DB->get_record('customfield_data', array('fieldid' => 15, 'instanceid' => $course));
    $language = $DB->get_record('customfield_data', array('fieldid' => 16, 'instanceid' => $course));
    $certificate = $DB->get_record('customfield_data', array('fieldid' => 17, 'instanceid' => $course));
    $objectives = $DB->get_records_sql('SELECT * FROM `mdl_customfield_data`WHERE `instanceid`=' . $course . ' and (`fieldid`=2 or `fieldid`=3 or `fieldid`=4 or `fieldid`=5 or `fieldid`=6 or `fieldid`=7 or `fieldid`=8 or `fieldid`=9 or `fieldid`=10)');
    $price = $DB->get_record('customfield_data', array('fieldid' => 12, 'instanceid' => $course));
    $view = $DB->get_record('course_views', array('courseid' => $course));
    $Allenrolusers = $DB->get_records_sql("SELECT count(*)as c From mdl_user_enrolments x,mdl_enrol y where x.enrolid=y.id and y.courseid='$course'");
    ...
    $user_context = $DB->get_record('context', array('instanceid' => $teacherId, 'contextlevel' => 30));
    ...
    $customFields = array(
        ...
        'price' => $price->value, 'views' => $view->visit, 'allenrolusers' => $allenrolusers,
        'activitiesCount' => $activitiesCount, 'teacherId' => $teacherId, 'teacherName' => $teacherName,
        'teacherBio' => '', 'duration' => "", 'promo' => $promo,
...
function course_view_data($course)
{
    ...
    try {
        ...
        return json_encode(["data" => $e]);
    }
}

function add_course_rate($userid, $course, $rate)
{
    global $DB;
    try {
        $context = context_course::instance($course, MUST_EXIST);
...
{
    return _get_courses_data($user, $categoryid);
}
...
function get_enrolled_users_members($courseid, $option)
{
    ...
    $notmemberData = array();
    $memberData = array();
    $group_members = $DB->get_records_sql("SELECT u.id As id,concat(u.firstname , ' ', u.lastname)as fullname ,op.empty as center_name ,op.school as school_name
    ...
    INNER JOIN mdl_user u ON ra.userid = u.id 
     LEFT OUTER JOIN mdl_optional_data_aibrahim as op ON op.userid=u.id

     WHERE cx.contextlevel = '50' AND c.id=$courseid   ");
    foreach ($group_members as $en_s) {
        if($en_s->center_name==null){
            $en_s->center_name='';
        }
        if($en_s->school_name==null){
            $en_s->school_name='';
        }
    }
    //     $flag = 0;
    ...
    return json_encode(['groupmember' => array_values($group_members), 'others' => $notmember]);
}
