                ...
                foreach ($modinfosections[$section->section] as $cmid) {
                    ...
                    if ($module['avail'] == false) {
                        ...
                        if (gettype($reason) == 'string') {
                            ...
                            $module['avail_message'] = get_string('list_root_and', 'theme_edumy');
                            $module['price'] = "count of reason items = " .count($reason->items);
                            for ($i = 0; $i < count($reason->items); $i++) {
                                if (gettype($reason->items[$i]) == 'string')
                                 {
                                    if (!strstr($reason->items[$i], 'Email')) 
                                    {
                                        $module['avail_message'] .= ' ' .strip_tags(\core_availability\info::format_info($reason->items[$i], $course)) .',';
                                    }                                    
                                 }
                                 else
                                 {
                                    $module['price'] .= " type = " .gettype($reason->items[$i]) .$i;
                                    $newreason = $reason->items[$i];
                                    echo json_encode($reason->items[$i]);
                    ...
                    if ($cm->modname == "resource2") {
                        ...
                        $video_type = $DB->get_record('reda_video_type2', array('resource2_id' => $cm->instance));
                        if ($video_type->type == "2") {
                            $module['resource_type'] = "lesson";
                        } elseif ($video_type->type == "1") {
                            $module['resource_type'] = "quiz";
