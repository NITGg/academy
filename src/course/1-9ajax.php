    ...
    try {
        ...
        if ($extension != "csv") {
            ...
            for ($i = 1; $i < count(getSheets($_FILES['excel']['tmp_name'])['Worksheet']); $i++) {
                ...
                $city = getSheets($_FILES['excel']['tmp_name'])['Worksheet'][$i][7];
                $school = getSheets($_FILES['excel']['tmp_name'])['Worksheet'][$i][8];
                $center = getSheets($_FILES['excel']['tmp_name'])['Worksheet'][$i][9];
                $parentFirstName = getSheets($_FILES['excel']['tmp_name'])['Worksheet'][$i][10];
                $parentLastName = getSheets($_FILES['excel']['tmp_name'])['Worksheet'][$i][11];
                $parentPassword = getSheets($_FILES['excel']['tmp_name'])['Worksheet'][$i][12];
                $parentEmail = getSheets($_FILES['excel']['tmp_name'])['Worksheet'][$i][13];
                $parentPhone = getSheets($_FILES['excel']['tmp_name'])['Worksheet'][$i][14];
                $parentRole = getSheets($_FILES['excel']['tmp_name'])['Worksheet'][$i][15];
                ...
                if (!empty($checkEmail)) {
                    ...
                    if ($_POST['teacherID'] != 72) {
                        ...
                        $courses = get_all_courses($userInfo->id, 72);
                        $count = count($courses);
                        for ($i = 0; $i < $count; $i++) {
                            $course = $DB->get_record('course', array('id' => $courses[$i]), '*', MUST_EXIST);
                            $context = context_course::instance($course->id);
                            $enrolmethod = 'manual';
                            if (!is_enrolled($context, $userInfo->id)) {
                                $enrol = enrol_get_plugin($enrolmethod);
