   ...
   LEFT OUTER JOIN   mdl_user u ON ra.userid = u.id WHERE cx.contextlevel = '50' AND c.id=" . $COURSE->id . "");
    ...
    if ($isadmin || $manager == 1) {
        ...
        if (!empty($teach)) {
            ...
            if (($add == "quiz" && $check_availability->quiz == "1") || ($add == "assign" && $check_availability->assign == "1") || ($add == "resource" && $check_availability->file == "1") || ($add == "page" && $check_availability->page == "1") || !empty($update) || ($add == "testnew" && $check_availability->pdf == "1")|| ($add == "url" && $check_availability->url == "1")) {
                $mform->display();
            } else {
                echo "
    ...
    if ($module->name == "resource" || $module->name == "resource2") {
        echo '<style>
        #id_submitbutton2{
            display:none;
        }
