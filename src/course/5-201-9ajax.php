    ...
    try {
        ...
        if ($extension != "csv") {
            ...
            for ($i = 1; $i < count(getSheets($_FILES['excel']['tmp_name'])['Worksheet']); $i++) {
                ...
                if (empty($email)) {
                    $failed[] = array('name' => $email, "reason" => "Empty Mail");
                } elseif (!empty($checkEmail)) {
                    $failed[] = array('name' => $email, "reason" => "Username Exists");
                } elseif (empty($firstname)) {
                    $failed[] = array('name' => $email, "reason" => "Empty First Name");
                } elseif (empty($lastname)) {
                    $failed[] = array('name' => $email, "reason" => "Empty Last Name");
                } elseif (empty($password)) {
                    $failed[] = array('name' => $email, "reason" => "Empty Password");
                } elseif ($role == 5 && empty($year)) {
                    $failed[] = array('name' => $email, "reason" => "no year specified");
                } else {
                    $notice = "";
                    if (!empty($phone1) && strlen($phone1) < 11) {
                        $notice .= " -Phone should be 11 digits ";
                    }
                    if (empty($phone2) || (!empty($phone2) && $phone2 != $parentPhone)) {
                        $check_parent = 0;
                        $notice .= " -Parent Phone in student not the same with phone in parent case so no parents will be added for this user ";
                    }
                    ...
                    if ($_POST['teacherID'] != 72) {
                        ...
                        $failed[] = array('name' => $email, "reason" => "success " . $notice);
                    } else {
                        $checks = array();
                        $user = $userInfo->id;
                    ...
                    if ($check_parent == 1) {
                        ...
                        if (empty($parentEmail)) {
                            $failed[] = array('name' => $parentEmail, "reason" => "Empty parent Mail");
                        } else {
                            $checkParentEmail = $DB->get_record('user', array('email' => $parentEmail));
                            if (empty($checkParentEmail)) {
                                if (empty($parentFirstName)) {
                                    $failed[] = array('name' => $parentEmail, "reason" => "Empty parent First Name" . $parentFirstName);
                                } elseif (empty($parentLastName)) {
                                    $failed[] = array('name' => $parentEmail, "reason" => "Empty parent Last Name" . $parentLastName);
                                } elseif (empty($parentPassword)) {
                                    $failed[] = array('name' => $parentEmail, "reason" => "Empty parent Password" . $parentPassword);
                                } elseif (empty($parentPhone) || (!empty($parentPhone) && strlen($parentPhone) < 11)) {
                                    $failed[] = array('name' => $parentEmail, "reason" => "Empty parent Last Name" . $parentPhone);
                                } else {
                                    ...
                                    $failed[] = array('name' => $parentEmail, "reason" => "success");
                                }
                            } else {
