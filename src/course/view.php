...
$videoRecords = $DB->get_records('course_promo_videos', array('course_id' => $id));
...
foreach ($videoRecords as $record) {
    $uri = "/videos/" . $record->url_name;
    $response = $client->request($uri, [], 'GET');
    $status = $response['body']['transcode']['status'];
    // Display the Vimeo player
    echo '<div class="vimeo-player">
    <video id="videoPlayer" width="100%" controls poster="../../services_imgs/course_promo_image.webp">
       <source src="' . $response['body']['files'][0]['link'] . '" type="video/mp4">
...
/* hide the course_content border */
echo '
<style>
.cs_row_three .course_content {
    border: none;
    border-color: rgb(237 239 247 / 0%);
}
</style>';

/* course panel title color */
echo '
<style>
...
/* course panel title background color */
echo '
<style>
.details .cc_tab h4.panel-title {
    border-radius: 15px;
    background: linear-gradient(166deg, #0E2647 32.94%, #B31F61 100%);
    color: white;
}
</style>';
...
#changenumsections .add-sections:hover {
