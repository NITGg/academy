...
import {
  previewCourseDiscount,
  startCourseCheckout,
  type CourseDiscountPreview,
} from "../actions";

interface BuyCourseModalProps {
  courseId: number;
  courseName: string;
  /** Effective price before any coupon (offer already applied), for the initial display. */
  basePrice: number;
...
export function BuyCourseModal({
  ...
  const [preview, setPreview] = useState<CourseDiscountPreview | null>(null);
  ...
  // Validate the coupon when the user finishes typing (blur / Enter / Apply)
  const checkCoupon = () => {
    setError(null);
    setPreviewing(true);
    previewCourseDiscount(courseId, coupon)
      .then((res) => {
        if (res.needsAuth) {
  ...
  const handlePay = () => {
    startPay(async () => {
      const res = await startCourseCheckout(courseId, coupon);
      if (res.needsAuth) {
        router.push(`/login?from=/courses/${courseId}`);
      } else if (res.error) {
        setError(res.error);
  ...
  // Prices: trust the server's preview when present, else the passed-in base price
  const finalPrice = preview?.final ?? basePrice;
  const strikePrice = preview
    ? preview.original !== preview.final
      ? preview.original
      : undefined
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="text-end">
            <h3 className="text-base font-bold text-foreground">شراء الكورس</h3>
            <p className="mt-0.5 line-clamp-1 text-xs text-muted-foreground">{courseName}</p>
          </div>
        </div>
        ...
        {error && (
          <p className="mt-3 rounded-lg bg-red-50 px-3 py-2 text-end text-[11px] text-red-600 dark:bg-red-900/20">
            {error}
          </p>
        )}
        ...
        <button
          ...
          className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-primary py-3.5 text-sm font-bold text-primary-foreground shadow-lg shadow-primary/20 transition hover:opacity-90 disabled:opacity-60"
        >
