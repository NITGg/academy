...
import {
  previewProgramDiscount,
  startProgramCheckout,
  type ProgramDiscountPreview,
} from "../actions";

interface BuyProgramModalProps {
  programId: number;
  programName: string;
  basePrice: number;
  currency?: string;
...
export function BuyProgramModal({
  programId,
  programName,
  basePrice,
  currency = "EGP",
  open,
  onClose,
}: BuyProgramModalProps) {
  const router = useRouter();
  const [coupon, setCoupon] = useState("");
  const [preview, setPreview] = useState<ProgramDiscountPreview | null>(null);
  ...
  const checkCoupon = () => {
    setError(null);
    setPreviewing(true);
    previewProgramDiscount(programId, coupon)
      .then((res) => {
        if (res.needsAuth) {
          router.push("/login");
          return;
  ...
  const handlePay = () => {
    startPay(async () => {
      const res = await startProgramCheckout(programId, coupon);
      if (res.needsAuth) {
        router.push("/login");
      } else if (res.error) {
        setError(res.error);
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="text-end">
            <h3 className="text-base font-bold text-foreground">الاشتراك في البرنامج</h3>
            <p className="mt-0.5 line-clamp-1 text-xs text-muted-foreground">{programName}</p>
          </div>
        </div>
