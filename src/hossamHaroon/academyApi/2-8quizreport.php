...
//  			 LEFT OUTER JOIN mdl_user u ON ra.userid = u.id WHERE cx.contextlevel = '50' AND c.id=$quiz->course");
...
function quiz_report_course($id, $option)
{
	...
	if ($option == "atoz") {
		...
		FROM mdl_course c LEFT OUTER JOIN mdl_context cx ON c.id = cx.instanceid 
		LEFT OUTER JOIN mdl_role_assignments ra ON cx.id = ra.contextid AND ra.roleid = '5'
		 LEFT OUTER JOIN mdl_user u ON ra.userid = u.id WHERE cx.contextlevel = '50' AND c.id=$quiz->course $option");
	} elseif ($option == "ztoa") {
	...
	op.empty as center ,op.school as school
	 ...
	 LEFT OUTER JOIN mdl_role_assignments ra ON cx.id = ra.contextid AND ra.roleid = '5'
	  LEFT OUTER JOIN mdl_user u ON ra.userid =  u.id 
	   LEFT OUTER JOIN mdl_logstore_standard_log as logs ON logs.userid=u.id 
	   INNER JOIN mdl_optional_data_aibrahim as op ON u.id=op.userid
		...
		op.empty as center ,op.school as school
		 ...
		 LEFT OUTER JOIN mdl_role_assignments ra ON cx.id = ra.contextid AND ra.roleid = '5'
		  LEFT OUTER JOIN mdl_user u ON ra.userid = u.id 
		   ...
		   WHERE cx.contextlevel = '50' AND c.id=$courseid  and logs.objectid=$objectid
			and logs.objecttable='resource' AND logs.action='viewed' GROUP BY logs.userid $option");
...
function course_report_quiz_user($courseid, $user)
{
   ...
   ");
	...
	// 				if($page->id==$pt->id){
	// 					$viewed="true";
	// 					$pageData[]=array('page_viewed'=>$viewed,"pageName"=>$page->pagename);
	// 				}
	// 				else{
	// 					$viewed="false";
	// 					$pageData[]=array('page_viewed'=>$viewed,"pageName"=>$pt->name);
	// 				}	
	// 		}
	// 		}
	// 		else{
	...
	$resources = $DB->get_records_sql("SELECT r.* FROM mdl_resource as r 
	join mdl_course_modules as cm ON cm.instance=r.id 
	join mdl_reda_video_type as rv on rv.resource_id=r.id
	where cm.course=$courseid and cm.module=18 and cm.deletioninprogress=0 and rv.type=1   ");
	...
	$codeData = array();
	$codeResult = array();
	$codes = $DB->get_records_sql("SELECT r.* FROM mdl_resource as r 
	join mdl_course_modules as cm ON cm.instance=r.id 
	join mdl_reda_video_type as rv on rv.resource_id=r.id
	where cm.course=$courseid and cm.module=18 and cm.deletioninprogress=0 and rv.type=2   ");
	$codes_views = $DB->get_records_sql("SELECT res.name as name,res.id as id
	FROM `mdl_logstore_standard_log` as logs
 	INNER JOIN mdl_user as user ON logs.userid=user.id 
 	INNER JOIN mdl_resource as res ON res.id=logs.objectid
	where courseid=$courseid AND user.id=$user AND action='viewed' and objecttable ='resource' and user.deleted=0  GROUP BY logs.objectid");
	if (!empty($codes)) {
		foreach ($codes_views as $file) {
			array_push($codeData, $file->id);
		}
	}
	foreach ($codes as $resource) {
		if (in_array($resource->id, $codeData)) {
			$views = resource_count($resource->id, $courseid, $user);
	...
	return json_encode(['quiz' => $quizData, 'exam' => $examData, 'page' => $pageResult, 'file' => $fileResult, 'lesson' => $codeResult]);
}
...
function course_report_quiz_user_average($courseid, $option, $quizids = array())
{
	...
	FROM mdl_course c LEFT OUTER JOIN mdl_context cx ON c.id = cx.instanceid 
	LEFT OUTER JOIN mdl_role_assignments ra ON cx.id = ra.contextid AND ra.roleid = '5'
 	LEFT OUTER JOIN mdl_user u ON ra.userid = u.id WHERE cx.contextlevel = '50' AND c.id=$courseid");
	...
	foreach ($enrolled_students as $user) {
		...
		if ($user_grades_count != 0) {
			...
			$quizData[] = array('user' => $user->id, 'fullname' => $fullname, "center_name" => $user_optional_data->empty, "school_name" => $user_optional_data->school, 'image' => $image, 'user_grades' => $user_grades_count);
			$user_grades_count = 0;
		} else {
			$fullname = $user->fname . " " . $user->lname;
			$user_grades_count = "0";
			...
			$noaverage[] = array('user' => $user->id, 'fullname' => $fullname, "center_name" => $user_optional_data->empty, "school_name" => $user_optional_data->school, 'image' => $image, 'user_grades' => $user_grades_count);
		}
	}
