...
function last_access_users($course)
{
	global $DB;
	try{
		...
		foreach ($Last_access_users as $last){
			...
			if($isStudent){
				$viewList[]=array('id'=>$last->id ,'name'=>$last->firstname.' '.$last->lastname,'lastaccess'=>date('m/d/Y H:i:s', $last->timeaccess),'centername'=>$centername);
	...
	foreach($enrolled_students as $en_s){
		if(!in_array($en_s->id,$users)){
			...
			$notViewList[]=array('id'=>$en_s->id ,'name'=>$en_s->fullname,'lastaccess'=>"0",'centername'=>$centername);
