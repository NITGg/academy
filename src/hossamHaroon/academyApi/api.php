...
function create_bbb_table($message,$messageKey,$hasBeenForciblyEnded,$duration,$hasUserJoined,$createDate,$dialNumber,$voiceBridge,$createTime,$moderatorPW,$attendeePW,$parentMeetingID,$internalMeetingID,$meetingID,$courseid,$isRunning){
		...
		function get_bbb_course_meetings($course){
			global $DB;
			$records=$DB->get_records_sql('SELECT meetingID from mdl_bbb_info where isRunning=0 and courseid='.$course.'');
		...
		function get_available_sessions(){
			$get_meetings_info="https://bbb.academy.nitg-eg.com/bigbluebutton/api/getMeetings?checksum=2408dde2a4706931ee63798a2cd633d9b510030b";
			$xml = file_get_contents($get_meetings_info);
		...
		function get_running_session($course){
			...
			if($last_record->isrunning=="1"){
				...
				$result='https://bbb.academy.nitg-eg.com/bigbluebutton/api/isMeetingRunning?meetingID='.$last_record->meetingid.'&checksum='.$val.'';
			...
			return json_encode(["bbb"=> 'NO SESSIONS from DB']);      
		...
		function create_new_bbb_session_parameters($course){
			global $DB;
		...
		function create_new_bbb_session($course,$user){
			...
			if($available=="true"){
			   ...
			   $logoutURL="https://academy.nitg-eg.com/bbb/end.php?course=".$course."";
			
				 $link= 'createname='.$name.'&meetingID='.$meetingID.'&attendeePW='.$attendeePW.'&moderatorPW='.$moderatorPW.'&maxParticipants='.$maxUsers.'&welcome='.$welcome.'&copyright='.$copyright.'&duration='.$duration.'&logoutURL='.$logoutURL.''.$sharedSecret.'';
				 $value=sha1($link);
				 $response='https://bbb.academy.nitg-eg.com/bigbluebutton/api/create?name='.$name.'&meetingID='.$meetingID.'&attendeePW='.$attendeePW.'&moderatorPW='.$moderatorPW.'&maxParticipants='.$maxUsers.'&welcome='.$welcome.'&copyright='.$copyright.'&duration='.$duration.'&logoutURL='.$logoutURL.'&checksum='.$value.'';
			  ...
			  $xml = file_get_contents($response, false, stream_context_create($arrContextOptions));
				//  $xml = file_get_contents($response);
				 ...
				 if($final['returncode']=="SUCCESS"){
					...
					if($join_link){
						...
						return json_encode(["bbb"=> 'Error Join ']);
					}
				 
				 }
				 else{
					return json_encode(["bbb"=> 'error Create']);
				 }
			}
		...
		function join_moderator($course,$user){
		   ...
		   $meetingID= $last_record->meetingid;
			...
			$value=sha1($check);
			$response='https://bbb.academy.nitg-eg.com/bigbluebutton/api/join?meetingID='.$meetingID.'&password='.$moderatorPW.'&fullName='.$user->username.'&checksum='.$value.'';
			return $response;
		}
		function join_attendee($course,$user){
			global $sharedSecret,$DB;
			$records=$DB->get_records_sql('SELECT * FROM `mdl_bbb_info`WHERE courseid='.$course.' and isRunning=1');
			$user=$DB->get_record('user',array('id'=>$user));
			$data=array_values($records);
		   ...
		   $meetingID= $last_record->meetingid;
			 ...
			 $value=sha1($check);
			 $response='https://bbb.academy.nitg-eg.com/bigbluebutton/api/join?meetingID='.$meetingID.'&password='.$attendeePW.'&fullName='.$user->username.'&checksum='.$value.'';
			 return  $response ;
		}
		function end_session($course){
			...
			$check='endmeetingID='.$meetingID.'&password='.$moderatorPW.''.$sharedSecret.'';
 			$value=sha1($check);
 			$response_end='https://bbb.academy.nitg-eg.com/bigbluebutton/api/end?meetingID='.$meetingID.'&password='.$moderatorPW.'&checksum='.$value.'';
			 $xml = file_get_contents($response_end);
			 $json=simplexml_load_string($xml);
			 $data=json_encode($json);
			 $final=json_decode($data,true);
	...
	function get_meeting_id($course){
		global $DB;
		$records=$DB->get_records_sql('SELECT * FROM `mdl_bbb_info`WHERE courseid='.$course.' and isRunning=1');
		$data=array_values($records);
	   $count=count($records);
	...
	function insert_create_time($course){
		global $DB;
