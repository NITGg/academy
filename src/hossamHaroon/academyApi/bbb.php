		...
		function create_new_bbb_session($course,$maxUsers=0,$user){
			...
			if ($sessionAvail == 'true'){
				...
				$res= $DB->insert_record('bbb_info', $ins);
					...
					$value=sha1($link);
					 $response='https://bbb.academy.nitg-eg.com/bigbluebutton/api/create?name='.$name.'&meetingID='.$meetingID.'&attendeePW='.$attendeePW.'&moderatorPW='.$moderatorPW.'&maxParticipants='.$maxUsers.'&welcome='.$welcome.'&copyright='.$copyright.'&duration='.$duration.'&checksum='.$value.'';
