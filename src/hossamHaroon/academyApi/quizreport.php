...
{
	global $DB;
	$records = $DB->get_records_sql('SELECT COUNT(*)as number FROM `mdl_logstore_standard_log` as logs INNER JOIN mdl_user as user ON logs.userid=user.id where courseid=' . $course . ' AND user.id=' . $user . ' AND action="viewed" and (objecttable ="resource" or objecttable ="resource2") and user.deleted=0 and objectid=' . $resource . ' GROUP BY userid
	');
	$views = 0;
...
function course_report_quiz_user($courseid, $user)
{
   ...
   ");
	...
	$resources = $resources + $DB->get_records_sql("SELECT r.* ,rv.type as type FROM mdl_resource2 as r 
	join mdl_course_modules as cm ON cm.instance=r.id 
	join mdl_reda_video_type2 as rv on rv.resource2_id=r.id
	where cm.course=$courseid and cm.module=25 and cm.deletioninprogress=0   ");
	...
	$files = $files + $DB->get_records_sql("SELECT res.name as name,res.id as id
	FROM `mdl_logstore_standard_log` as logs
	INNER JOIN mdl_user as user ON logs.userid=user.id 
	INNER JOIN mdl_resource2 as res ON res.id=logs.objectid
	where courseid=$courseid AND user.id=$user AND action='viewed' and objecttable ='resource2' and user.deleted=0  GROUP BY logs.objectid");
