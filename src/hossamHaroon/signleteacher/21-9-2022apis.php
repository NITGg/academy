    ...
    function register($firstname, $lastname, $username, $email, $password, $phone = null, $phone2 = null, $role, $year = null, $city = null, $school = null, $center = null)
    {
        ...
        if (!empty($check_userrname)) {
            ...
            try {
                ...
                $url = $image;
                $courseData = $this->get_all_courses($userInfo->id, 15);//change the teacher id on each api
                $enrol=$this->enrol_student($courseData,$userInfo->id,5);
                $userData = array(
        ...
        try {
           ...
           LEFT OUTER JOIN   mdl_user u ON ra.userid = u.id WHERE cx.contextlevel = '50' AND u.id= " . $teacherid . "";
            ...
            foreach ($courses as $key => $record) {
                ...
                } elseif ($userid == $teacherid && ($endtime == 0 || $_SERVER['REQUEST_TIME'] < $endtime)||$isAssistant) {
                    if ($status == 0) {
                        $courses_data[] = array(
                            "courseId" => $record->courseid, "courseName" => $record->coursename,
                            "courseDesc" => format_string($record->course_desc),
...
function aboutInfo()
{
    ...
    على كل حال ... أهلا بكم  في هذا الملتقى التعليمي المتميز  لنتعاون جميعا من خلال منظومة إلكترونية متخصصة للوصول إلى أعلى الدرجات في مادة اللغة العربية مع الأستاذ العالمي حسام هارون.
    </p>
