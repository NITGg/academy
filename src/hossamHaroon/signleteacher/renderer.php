...
<div class="alert alert-warning">please add code to open it</div>
                ...
                <div class="content">
                    ...
                    <input type="text" class="form-control col-lg-5" placeholder="Add a Code" id="codeGroup">
                    <button class="btn btn-primary col-lg-7" id="addCode">' . get_string("courseAddCode", "theme_edumy") . '</button>
                    </div>
                    <div class="content2">       
                    </div>
                </div>
  ...
  ';
            $output .= html_writer::tag('a', $activitylink, array('class' => "popup-with-zoom-anim", " href" => "#small-dialog", 'id' => $mod->id));
            $output1 = $mod->availableinfo;
            if (strpos($output1, 'week') !== false) {
                $output .= "<script>
            ...
            $(document).ready(function () {
                ...
                $('#success').hide();
                  $('#addCode').click(function(){
                      ...
                      $.ajax('/ajax.php', {
                type: 'POST',  
                data: { group: codeGroup,userid: " . $USER->id . ",courseGroup:".$COURSE->id."},  
               ...
               if(data == 'success'){
                ...
                $('.popup-with-zoom-anim').magnificPopup({
                    ...
                    callbacks: {
                        resize: function() {
                            var smallDlg = $('#small-dialog');
                            
                            var img = $('.image_container img');
                            var imgH = img.outerHeight();
                            
                            var con1 = $('.content .content1');
                            var con1H = con1.outerHeight();
                        }
                    }
                });
            });
