<?php
// Admin UI to manage discount coupons (US-AD-7-1 create, US-AD-7-2 update, US-AD-7-3 deactivate/
// delete). Uses the local_academy HTTP API (api.php) from the browser with a mobile web-service
// token minted for the logged-in admin. Mirrors manage_subscriptions.php.
...
admin_externalpage_setup('local_academy_managecoupons');
require_capability('local/academy:managecoupons', context_system::instance());
...
$PAGE->set_title(get_string('managecoupons', 'local_academy'));
$PAGE->set_heading(get_string('managecoupons', 'local_academy'));
$PAGE->requires->js(new moodle_url('/local/academy/ui.js'), true);

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('managecoupons', 'local_academy'));
