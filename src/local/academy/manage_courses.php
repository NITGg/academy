...
// purchase, unenrolling the buyer. Mirrors the "User Subscriptions" section of
// manage_subscriptions.php (token + inline JS + AcademyUI.paginate). Reuses the subscriptions
// capability so no new capability/DB migration is required.
...
admin_externalpage_setup('local_academy_managecourses');
...
$PAGE->set_title(get_string('managecourses', 'local_academy'));
$PAGE->set_heading(get_string('managecourses', 'local_academy'));
$PAGE->requires->js(new moodle_url('/local/academy/ui.js'), true);

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('managecourses', 'local_academy'));
...
$STR = local_academy_string_map(array(
    ...
    'mc_heading', 'mc_desc', 'mc_col_course', 'mc_col_purchased', 'mc_none',
