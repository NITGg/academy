<?php
// Admin UI to manage automatic offers (US-AD-8-1 create, US-AD-8-2 update, US-AD-8-3 deactivate/
// delete). Uses the local_academy HTTP API (api.php) with a mobile web-service token minted for the
// logged-in admin. Mirrors manage_coupons.php (offers carry no code / max / usage limit).
...
admin_externalpage_setup('local_academy_manageoffers');
require_capability('local/academy:manageoffers', context_system::instance());
...
$PAGE->set_title(get_string('manageoffers', 'local_academy'));
$PAGE->set_heading(get_string('manageoffers', 'local_academy'));
$PAGE->requires->js(new moodle_url('/local/academy/ui.js'), true);

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('manageoffers', 'local_academy'));
...
$STR = local_academy_string_map(array(
    ...
    'cpn_type_fixed', 'cpn_scope_courses', 'cpn_scope_packages', 'cpn_scope_subscriptions',
