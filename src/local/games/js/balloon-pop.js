...
(function () {
    ...
    window.LocalGames.register('balloon-pop', function (api) {
         ...
         * Pick a short rule.
         *
         * @return {{test: Function, label: string}}
         */
        function makeRule() {
            var kind = api.pick(['even', 'odd', 'greater', 'less', 'divisible']);
            var n;
            ...
            if (kind === 'even') {
                return {
                    ...
                    label: api.strings.balloon_rule_even
                };
            }
            if (kind === 'odd') {
                return {
                    ...
                    label: api.strings.balloon_rule_odd
                };
            }
            if (kind === 'greater') {
                n = api.random(10, 25);
                return {
                    ...
                    label: api.strings.balloon_rule_greater.replace('{$a}', api.fmt(n))
                };
            }
            if (kind === 'less') {
                n = api.random(10, 25);
                return {
                    ...
                    label: api.strings.balloon_rule_less.replace('{$a}', api.fmt(n))
                };
            }
            n = api.pick([2, 3, 5]);
            return {
                ...
                label: api.strings.balloon_rule_divisible.replace('{$a}', api.fmt(n))
            };
        }
