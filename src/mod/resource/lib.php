...
function resource_delete_instance($id) {
    ...
    $vimeo_files = $DB->get_record('vimeo_files', array('resource_id' => $resource->id));
    $type=$DB->get_record('reda_video_type', array('resource_id' => $resource->id));
    if(!empty($type)){
        $DB->delete_records('reda_video_type', array('resource_id' => $resource->id));
    }
