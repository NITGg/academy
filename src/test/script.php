...
elseif(isset($_POST['update_form'])){
	...
	try{
		...
		$typeData=$DB->get_record('reda_video_type',array('resource_id'=>$id));
		$type=$typeData->type;
		$video_type=new stdClass();
		$video_type->id=$typeData->id;
		$video_type->type=$_POST['type'];
		$DB->update_record('reda_video_type',$video_type);
		// echo $_POST['video_type'];
	}
	catch(Exception $e){
